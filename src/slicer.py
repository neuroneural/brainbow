"""
Custom slicer-plotter class based on nibabel's OrthoSlicer3D
"""
...
class CustomSlicer(object):
    """
    Orthogonal-plane slice viewer based on nibabel's OrthoSlicer3D.
    """
    ...
    def __init__(
        ...
        underlay_data = np.asanyarray(underlay_data)
        if underlay_data.ndim != 3:
            raise ValueError("underlay_data must have 3 dimensions")
        if np.iscomplexobj(underlay_data):
            raise TypeError("Complex data (in underlay) not supported")
        overlay_data = np.asanyarray(overlay_data)
        if overlay_data.ndim != 4:
            raise ValueError("overlay_data must have 4 dimensions")
        if np.iscomplexobj(overlay_data):
            raise TypeError("Complex data (in overlay) not supported")

        underlay_affine = np.array(underlay_affine, float)
        if underlay_affine.shape != (4, 4):
            raise ValueError("Underlay affine must be a 4x4 matrix")
        overlay_affine = np.array(overlay_affine, float)
        if overlay_affine.shape != (4, 4):
            raise ValueError("Overlay affine must be a 4x4 matrix")

        # determine our orientation
        self._underlay_affine = underlay_affine
        self._overlay_affine = overlay_affine
        underlay_codes = axcodes2ornt(aff2axcodes(self._underlay_affine))
        self._underlay_order = np.argsort([c[0] for c in underlay_codes])
        self._underlay_flips = np.array([c[1] < 0 for c in underlay_codes])[
            self._underlay_order
        ]
        self._underlay_flips = list(self._underlay_flips) + [False]  # add volume dim
        self._underlay_scalers = voxel_sizes(self._underlay_affine)
        self._underlay_inv_affine = np.linalg.inv(underlay_affine)
        # current volume info
        self._underlay_volume_dims = underlay_data.shape[3:]
        # self._underlay_current_vol_data = data[:, :, :, 0] if data.ndim > 3 else underlay_data
        self._underlay_current_vol_data = underlay_data
        self._underlay_data = underlay_data
        self._underlay_clim = np.percentile(underlay_data, (1.0, 99.0))
        del underlay_data
...
class OrthoSlicer3D(object):
    ...
    def __init__(self, data, affine, axes, position, underlay, imshow_args, title=None):
        """
        Parameters
        ----------
        data : array-like
        ...
        for ii, xax, yax, ratio, label in zip(
            ...
            d = np.zeros((self._sizes[yax], self._sizes[xax]))
            if underlay:
                # print(f"Underlay size is {d.shape}")
                im = self._axes[ii].imshow(
                    d,
                    vmin=self._clim[0],
                    vmax=self._clim[1],
                ...
                # print(f"Overlay size is {d.shape}")
                im = self._axes[ii].imshow(
                    d,
                    # vmin=self._clim[0],
                    # vmax=self._clim[1],
